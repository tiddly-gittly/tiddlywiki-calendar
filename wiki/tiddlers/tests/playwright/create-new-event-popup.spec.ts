...
const openEventCalendarLayout = async (page: import('@playwright/test').Page) => {
  ...
  await page.locator('a[href*="EventsCalendarLayout"]').first().click();
...
test('new event popup autofocuses caption after drag create', async ({ page }) => {
  ...
  const captionInput = page.locator('.tw-calendar-layout-create-new-tiddler-popup .tw-calendar-caption-input');
...
test('recurrence quick actions apply COUNT and UNTIL rules', async ({ page }) => {
  ...
  await page.locator('.tw-calendar-details-summary').first().click();
...
test('until current occurrence uses clicked recurring instance time', async ({ page }) => {
  ...
  await expect.poll(async () => readTiddlerField(page, recurringTitle, 'rrule')).toMatch(/20260428/);
});
...
test('existing recurring event pre-fills interval and count from rrule', async ({ page }) => {
  ...
  await page.evaluate((title) => {
    ...
    tw.wiki.addTiddler({
      title,
      caption: title,
      startDate: '20260428120000000',
      endDate: '20260428130000000',
      calendarEntry: 'yes',
